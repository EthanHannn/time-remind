...
当前版本可以作为 `0.1.6 Beta` 发给少量可信用户试用。
...
- NSIS installer: `src-tauri/target/release/bundle/nsis/Time Remind_0.1.6_x64-setup.exe`
...
- macOS `.dmg`: `src-tauri/target/release/bundle/dmg/Time Remind_0.1.6_*.dmg`
- macOS ARM64 GitHub Actions artifact: `Time-Remind_0.1.6_macOS_aarch64_app.zip`
- macOS ARM64 GitHub Actions DMG diagnostics: `Time-Remind_0.1.6-macos-aarch64-dmg-diagnostics`
...
7. 为安装包生成 SHA256 校验值。
8. 创建版本提交并推送到 GitHub。
9. 创建并推送 `v0.1.6` 形式的标签，触发 GitHub Actions 自动发布构建。
10. 检查 Release 说明是否明确标注 Windows 已验证、macOS/Linux 为社区预览。
11. 分发 `Time Remind_0.1.6_x64-setup.exe` 和对应校验值给试用用户。
