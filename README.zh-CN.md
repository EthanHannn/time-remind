...
| macOS | 社区预览，未验证 | 会随发布提供预览包，用于开源反馈；托盘、自启动、锁屏和全屏行为仍需实机验证。 |
| Linux | 社区预览，未验证 | 会随发布提供预览包，用于开源反馈；托盘兼容性和桌面环境差异仍需验证。 |

Windows 是当前已验证发布平台。macOS 和 Linux 安装包属于社区预览，在完成平台能力降级和实机验证前，不视为完整支持。
...
当前 Windows 安装包未做代码签名，可能提示未知发布者。macOS 预览包暂未签名和公证，可能出现 Gatekeeper 安全提示。
...
- macOS 和 Linux 产物在验证完成前必须明确标注为社区预览资产。
...
- macOS 和 Linux 构建在完成验证清单前仍作为预览资产。
